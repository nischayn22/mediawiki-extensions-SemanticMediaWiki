...
/**
 * Class Handling all the write and update methods for SMWSQLStore
 *
...
Class SMWSQLStore2Writers {
	...
	public function doDataUpdate( SMWSemanticData $data ) {
		...
		$this->store->m_semdata[$sid] = SMWSqlStubSemanticData::newFromSemanticData( $data );
		...
		foreach ( $data->getProperties() as $property ) {
			...
			foreach ( $data->getPropertyValues( $property ) as $di ) {
				...
				if ( $di instanceof SMWDIContainer ) { // process subobjects recursively
					...
					$handler = SMWDataItemHandler::getDataItemHandlerForDIType( $di->getDIType(), $this->store );
					$uvals = array_merge( $uvals, $handler->getInsertValues( $di ) );
				}
